...
export class CartPage extends Component {
  ...
  render() {
    ...
    this.state.cart_products.forEach(CurrentProduct => {
      if (CurrentProduct.ProductDiscount !== '0' && CurrentProduct.product_isselected === true) {
        TotalDiscount += (Number.parseFloat(CurrentProduct.ProductPriceBefore) - Number.parseFloat(CurrentProduct.product_price)) * Number.parseFloat(CurrentProduct.cart_count);
      }
    });
            ...
            <div class="PageSideContainer">
              {TotalProducts > 0 &&
                <div class="CartSummaryContainer">
                  ...
                  <div class="CartSummaryInfoContainer">
                    <h class="SideContainerText">Доставка</h>
                    <h class="SideContainerText">{new Intl.NumberFormat().format(DelieveryPrice) + " ₽"}</h>
                  </div>
                  ...
                  <h class="SideContainertDelieveryDate">Доставим {DelieveryDate}</h>
                </div>
              }
            </div>
          </div>
