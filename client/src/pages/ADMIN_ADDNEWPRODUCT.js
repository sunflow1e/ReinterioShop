...
export class ADMIN_ADDNEWPRODUCT extends Component {
	...
	getImages() {
		...
		let config = {
			...
			maxBodyLength: Infinity,
			url:
				`${process.env.REACT_APP_API_URL}/admin/allimages/` +
	...
	getFiles() {
		...
		let config = {
			...
			maxBodyLength: Infinity,
			url:
				`${process.env.REACT_APP_API_URL}/admin/file/` +
				Number.parseInt(this.state.ProdId),
			headers: {},
			data: data,
		}
	...
	render() {
		...
		ColorsImages.map(el =>
			el.productd_id === null
				? (el.productd_id = Math.floor(
						Math.random() * (1000000 - 50000 + 1) + 50000
				  ))
				: (el.productd_id = el.productd_id)
		)
						...
						>
							...
							>
								...
								<p className='PageCardText'>
									Загрузите фотографии под каждый отдельный цвет
								</p>
								{ColorsImages.map(productcolor => (
									<ADMIN_ADDP_IMAGES
										...
										images={this.state.images.filter(
											el => el.productd_id === productcolor.productd_id
										)}
									/>
								))}
							</div>
							...
							>
								...
								<p style={{ marginBottom: '10px' }} className='PageCardText'>
									Добавьте файлы со справочной и иной информацией, которая может
									пригодиться пользователям
								</p>
								...
								<div className='ModalWindow'>
									...
									<div className='ModalContainer'>
										...
										<div className='ModalButtonsContainer'>
											<div
												onClick={() =>
													this.setState({ SuccessModalShown: false })
												}
								...
								<div className='ModalWindow'>
									...
									<div className='ModalContainer'>
										...
										<p style={{ lineHeight: '150%', fontSize: '18px' }}>
											Вы уверены, что хотите сохранить товар? Удалить новые
											цвета товара будет невозможно
										</p>
										<div className='ModalButtonsContainer'>
											<div
												onClick={() => this.closeModalWindow()}
	...
	saveProductToBD() {
		...
		this.state.styles
			.filter(el => el.style_ischecked === true)
			.map(el => this.addStyles(el.style_id))
		this.state.materials
			.filter(el => el.material_ischecked === true)
			.map(el => this.addMaterials(el.material_id))
		this.state.files.map(el => this.addFiles(el))

		for (const el of this.state.colors.filter(
			el => el.color_ischecked === true && el.color_readonly === false
		)) {
			this.addColors(el)
		}
	...
	postProduct() {
		let disountcprice = Number.parseFloat(
			document.getElementById('product_price' + this.state.ProdId).value
		)
		let discount = Number.parseFloat(
			document.getElementById('product_discount' + this.state.ProdId).value
		)
		...
		let data = qs.stringify({
			...
			length: document.getElementById('product_lenght' + this.state.ProdId)
				.value,
			width: document.getElementById('product_width' + this.state.ProdId).value,
			height: document.getElementById('product_height' + this.state.ProdId)
				.value,
			weight: document.getElementById('product_weight' + this.state.ProdId)
				.value,
	...
	addColors(color) {
		...
		let data = qs.stringify({
			...
			onstock: document.getElementById('product_onstock' + color.color_id)
				.value,
		})
		...
		axios
			...
			.then(response => {
				this.doThis(color)
				for (const el of this.state.images.filter(
					el => el.productd_id === color.productd_id
				)) {
					this.uploadNewImages(el.image_path)
				}
			})
			.catch(error => {
				console.log(error)
	...
	checkAllFilled() {
		if (
			document.getElementById('product_name' + this.state.ProdId).value === ''
		) {
		...
		var inputprice = document.getElementById(
			'product_price' + this.state.ProdId
		)
		...
		if (
			...
			this.setState({
				ErrorText:
					'Заполните поле скидки. Чтобы товар был без скидки, поставьте 0',
			})
			return false
		}

		var inputdiscount = document.getElementById(
			'product_discount' + this.state.ProdId
		)

		if (!inputdiscount.checkValidity()) {
			this.setState({ ShowError: true })
			this.setState({
				ErrorText: 'Поле скидки должно содержать значение от 0 до 99',
			})
			return false
		}

		if (
			document.getElementById('product_weight' + this.state.ProdId).value === ''
		) {
			this.setState({ ShowError: true })
		...
		var inputweight = document.getElementById(
			'product_weight' + this.state.ProdId
		)
		...
		if (!inputweight.checkValidity()) {
			...
			this.setState({
				ErrorText:
					'Поле веса должно содержать только число. Для нецелого значения используйте точку (2.3)',
			})
			return false
		}

		if (
			document.getElementById('product_width' + this.state.ProdId).value === ''
		) {
			this.setState({ ShowError: true })
		...
		var inputwidth = document.getElementById(
			'product_width' + this.state.ProdId
		)
		...
		if (!inputwidth.checkValidity()) {
			...
			this.setState({
				ErrorText:
					'Поле ширины должно содержать только число. Для нецелого значения используйте точку (2.3)',
			})
			return false
		}

		if (
			document.getElementById('product_height' + this.state.ProdId).value === ''
		) {
			this.setState({ ShowError: true })
		...
		var inputheight = document.getElementById(
			'product_height' + this.state.ProdId
		)
		...
		if (!inputheight.checkValidity()) {
			...
			this.setState({
				ErrorText:
					'Поле высоты должно содержать только число. Для нецелого значения используйте точку (2.3)',
			})
			return false
		}

		if (
			document.getElementById('product_lenght' + this.state.ProdId).value === ''
		) {
			this.setState({ ShowError: true })
		...
		var inputlenght = document.getElementById(
			'product_lenght' + this.state.ProdId
		)
		...
		if (!inputlenght.checkValidity()) {
			...
			this.setState({
				ErrorText:
					'Поле длины должно содержать только число. Для нецелого значения используйте точку (2.3)',
			})
			return false
		}
		...
		if (
			this.state.colors.filter(el => el.color_ischecked === true).length < 1
		) {
			this.setState({ ShowError: true })
		...
		if (
			this.state.materials.filter(el => el.material_ischecked === true).length <
			1
		) {
			this.setState({ ShowError: true })
		...
		if (
			this.state.styles.filter(el => el.style_ischecked === true).length < 1
		) {
			this.setState({ ShowError: true })
		...
		for (let index = 0; index < ColorsImages.length; index++) {
			if (
				this.state.images.filter(
					el => el.productd_id === ColorsImages[index].productd_id
				).length < 1
			) {
				this.setState({ ShowError: true })
				this.setState({
					ErrorText:
						'Необходимо добавить хотя бы одну фотографию на каждый цвет',
				})
				return false
			}
		}
		...
		for (let index = 0; index < ColorsImages.length; index++) {
			if (
				document.getElementById(
					'product_onstock' + ColorsImages[index].color_id
				).value === ''
			) {
				this.setState({ ShowError: true })
				this.setState({
					ErrorText: 'Необходимо добавить количество товара на складе',
				})
				return false
			}
			...
			if (
				!document
					.getElementById('product_onstock' + ColorsImages[index].color_id)
					.checkValidity()
			) {
				this.setState({ ShowError: true })
				this.setState({
					ErrorText: 'Поле «В наличии» должно содержать только цифры',
				})
				return false
			}
		}
	...
	addNewFile() {
		let objectToAdd = [
			{
				file_id: Math.floor(Math.random() * (1000000 - 50000 + 1) + 50000),
				file_path: null,
				file_name: null,
				file_product: this.ProdId,
			},
		]
	...
	saveFile(filepath, fileid) {
		const productindex = this.state.files.map(a => a.file_id).indexOf(fileid)
	...
	uploadImage(id, file_path) {
		let objectToAdd = [
			{ image_path: file_path, productd_id: Number.parseInt(id) },
		]
	...
	changeSelectedStyle(styleid, selection) {
		const productindex = this.state.styles.map(a => a.style_id).indexOf(styleid)
	...
	changeSelectedColor(colorid, selection) {
		const productindex = this.state.colors.map(a => a.color_id).indexOf(colorid)
