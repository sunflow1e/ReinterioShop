...
export class ADMIN_ADDNEWPRODUCT extends Component {
    ...
    async saveProductToBD() {
        ...
        this.state.colors.filter(el => el.color_ischecked === true).map(async el => await this.addColors(el))
    ...
    async addColors(color) {
        ...
        axios.request(config)
            .then(async (response) => {
                this.state.images.filter(el => el.productd_id === color.productd_id).map(async el => await this.uploadNewImages(el.image_path))
            })
            .catch((error) => {
                console.log(error);
            });
    ...
    async uploadNewImages(path) {
        ...
        axios.request(config)
            .then(async (response) => {
                this.state.colors.filter(el => el.color_ischecked === true).map(async el => await this.updateCovers())
            })
            .catch((error) => {
                console.log(error);
            });
